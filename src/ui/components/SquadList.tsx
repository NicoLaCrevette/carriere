...
import type { CouleursClub } from '../lib/avatar';

interface SquadListProps {
  byPosition: Partial<Record<Position, (Player | NpcPlayer)[]>>;
  playerId: Id;
  asOf: ISODate;
  /** Couleurs du club, portées par les portraits de l'effectif. */
...
export default function SquadList({ byPosition, playerId, asOf, couleurs }: SquadListProps) {
  return (
    <div className="space-y-4">
      {POSITIONS.map((pos) => {
        ...
        return (
          <div key={pos}>
            <h4 className="text-[11px] uppercase tracking-wide text-muted mb-1">{POSITION_LABELS[pos]}</h4>
            <ul className="divide-y divide-white/[0.05] text-sm">
              {rows.map((p) => {
                ...
                return (
                  <li key={p.id} className={`flex items-center gap-3 py-1.5 ${p.id === playerId ? 'text-accent' : ''}`}>
                    <PlayerAvatar
                      {...(isPlayer(p) && p.identity.avatar ? { config: p.identity.avatar } : { id: p.id })}
                      {...(couleurs ? { couleurs } : {})}
                      taille={28}
                      anneau={p.id === playerId}
                    />
                    <span className="flex-1 truncate">
                      {p.identity.firstName} {p.identity.lastName}
                    </span>
                    <span className="w-10 text-right text-muted"><NumberTabular value={ageAt(p.identity.birthDate, asOf)} /></span>
                    <span className="w-10 text-right font-semibold"><NumberTabular value={p.overall} /></span>
                    <span className="w-14 text-right text-muted"><NumberTabular value={p.form >= 0 ? `+${p.form}` : p.form} /></span>
                    <span className="w-24 text-right">
                      {injured && <span className="text-signal-red text-[11px] uppercase">Blessé</span>}
                      {!injured && suspended && <span className="text-signal-red text-[11px] uppercase">Suspendu</span>}
                    </span>
                  </li>
                );
              })}
